...
Mapper.prototype.run = function run () {
  ...
  console.log('created node, waiting list is', this.waiting);
  setInterval(function() {
    //every 2 seconds pop a node and call it if possible.
    if(this.waiting.length > 0) {
      console.log('some nodes waiting', this.waiting.length);
...
Mapper.prototype._generateCallMadeHandler = function(node, mapper) { 
  ...
  return function(err, call) {
    console.log('call started' err, call)
    if(err) {
      node.error = err;
...
Mapper.prototype._generateRecordingHandler = function (numberMatches, currentSid, mapper) {
  return function(err, data) {
    console.log('recording callback got', err, data);
