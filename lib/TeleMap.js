...
function _nodesToString(arr) {
  var output = "\n";
  for(var i=0; i<arr.length; i++) {
    output += arr[i].number;
    output += ' ext ';
    output += arr[i].toneString;
    output += "\n";
  }
  return output;
}
...
Mapper.prototype.run = function run () {
  ...
  setInterval(function() {
    ...
    if(this.waiting.length > 0) {
      console.log('waiting nodes ...', _nodesToString(this.waiting));
...
Mapper.prototype._initServer = function () {
  ...
  self.server.post({ path : '/transcript' } , self._handleTranscribeCallback.bind(self));
...
Mapper.prototype.mapNode = function (node) {
  ...
  console.log('mapNode, id:', node.id, node.number);
  this.activeRoutes[node.id] = this.server.get({ path : node.getTWIMLPath() }, this._generateXmlEndpoint(node));
  console.log('created path', node.getTWIMLPath(), 'for node', node.number, node.toneString);
...
Mapper.prototype._generateCallMadeHandler = function(node, mapper) { 
  ...
  return function(err, call) {
    if(err) {
      ...
      console.log('calling ...', node.number, ' ext ', node.toneString);
...
Mapper.prototype._handleTranscribeCallback = function (req, res, next) {
  ...
  // var regex = /([0-9#*]|star)/g;
  var regex = /([0-9#*]|\bstar\b|\bpound\b)/g;
  console.log('transcript text: ', response_obj.TranscriptionText);
  var numberMatches = _.uniq(response_obj.TranscriptionText.match(regex)); //unique number matches
  numberMatches = numberMatches.map(function(number) {
    //transform star and pound to their characters
    if(number == 'star') return '*';
    if(number == 'pound') return '#';
    return number;
  });
  var currentSid = response_obj.CallSid;
  console.log('call sid', currentSid, 'got transcript callback', numberMatches);
  this.twilio.recordings.list({ callSid : currentSid }, this._generateRecordingHandler(numberMatches, currentSid, this));
}

Mapper.prototype._generateRecordingHandler = function (numberMatches, currentSid, mapper) {
  console.log('generating recording handler function for sid', currentSid);
  return function(err, data) {
    ...
    if(recording) {
      ...
      console.log('recording of duration', recording.duration, 'has keys', numberMatches, 'wait chunk: ', waitChunk);
      for(var i=0; i<numberMatches.length; i++) {
        ...
        console.log(waitTime,' secs + ', numberMatches[i], 'turns into', childToneString);
        
        this.calling[currentSid] = undefined;
        this.done[currentSid] = parentNode;
      }
      console.log('created children', this.waiting);
      console.log('done nodes', this.done);
    }
  }.bind(mapper)
}
